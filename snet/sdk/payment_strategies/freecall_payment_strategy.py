...
class FreeCallPaymentStrategy(PaymentStrategy):
    ...
    def get_free_calls_available(self, service_client) -> int:
        ...
        if (
            not self._free_call_token
            or not self._token_expiration_block
            or current_block_number > self._token_expiration_block
        ):
            self._free_call_token, self._token_expiration_block = self.get_free_call_token_details(
                service_client
            )
    ...
    def get_payment_metadata(self, service_client) -> list:
        if self.get_free_calls_available(service_client) <= 0:
            raise Exception(
                f"Free calls limit for address {self._user_address} has expired. Please use another payment strategy"
            )
        signature, current_block_number = self.generate_signature(service_client)
        metadata = [
            ("snet-free-call-auth-token-bin", self._free_call_token),
    ...
    def generate_signature(
        self, service_client, current_block_number=None, with_token=True
    ) -> tuple[bytes, int]:
    ...
    def get_free_call_token_details(
        self, service_client, current_block_number=None
    ) -> tuple[bytes, int]:

        signature, current_block_number = self.generate_signature(
            service_client, current_block_number, with_token=False
        )
