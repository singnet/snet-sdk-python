...
from snet.sdk.payment_strategies.training_payment_strategy import (
    TrainingPaymentStrategy,
)
...
class Training:
    ...
    def create_model(
        ...
        new_model = self.training.NewModel(
            name=model_name,
        ...
        create_model_request = self.training_daemon.NewModelRequest(
            authorization=auth_details, model=new_model
        )
        response = self._call_method("create_model", request_data=create_model_request)
    ...
    def validate_model_price(self, model_id: str) -> int:
        ...
        validate_model_price_request = self.training_daemon.AuthValidateRequest(
            authorization=auth_details, model_id=model_id, training_data_link=""
        )
    ...
    def train_model_price(self, model_id: str) -> int:
        ...
        common_request = self.training_daemon.CommonRequest(
    ...
    def get_training_metadata(self) -> TrainingMetadata:
        ...
        response = self._call_method("get_training_metadata", request_data=empty_request)

        training_metadata = TrainingMetadata(
            training_enabled=response.trainingEnabled,
    ...
    def get_all_models(
        ...
        response = self._call_method("get_all_models", request_data=all_models_request)
        models = []
    ...
    def get_method_metadata(self, method_name: str, model_id: str = "") -> MethodMetadata:
        ...
        if model_id:
            requirements_request = self.training_daemon.MethodMetadataRequest(
                grpc_method_name="", grpc_service_name="", model_id=model_id
            )
        else:
            service_name, method_name = self._check_method_name(method_name)
            requirements_request = self.training_daemon.MethodMetadataRequest(
        ...
        response = self._call_method("get_method_metadata", request_data=requirements_request)
    ...
    def upload_and_validate(
        self, model_id: str, zip_path: str | Path | PurePath, price: int
    ) -> ModelStatus:
        f = open(zip_path, "rb")
        ...
        def request_iter(file):
            ...
            while batch:
                ...
                yield self.training_daemon.UploadAndValidateRequest(
                    authorization=auth_details, upload_input=upload_input
                )
                batch = file.read(batch_size)
                batch_number += 1
    ...
    def _call_method(self, method_name: str, request_data, paid=False) -> Any:
        try:
    ...
    def _get_auth_details(self, method_msg: ModelMethodMessage) -> Any:
        ...
        signature = self.service_client.generate_training_signature(
            method_msg.value, address, current_block_number
        )
    ...
    def _get_grpc_channel(self, base_channel: grpc.Channel) -> grpc.Channel:
        intercept_call_func = create_intercept_call_func(
            self.payment_strategy.get_payment_metadata, self.service_client
        )
        grpc_channel = grpc.intercept_channel(
            base_channel, generic_client_interceptor.create(intercept_call_func)
        )
        return grpc_channel
