...
class MPEServiceMetadata:
    ...
    def add_contributor(self, name, email_id):
        ...
        contributors.append({"name": name, "email_id": email_id})
        self.m["contributors"] = contributors

    def remove_contributor_by_email(self, email_id):
        self.m["contributors"] = [
            contributor
            for contributor in self.m["contributors"]
            if contributor["email_id"] != email_id
        ]
    ...
    def add_media(self, url, media_type, hero_img=False):
        ...
        if hero_img:
            ...
            assert not self._is_asset_type_exists(), (
                "Hero-image already exists (only 1 unique hero-image allowed.)"
            )
            individual_media["asset_type"] = (
                AssetType.HERO_IMAGE.value
            )  # Dependency with AssetType, fix if obsolete
    ...
    def swap_media_order(self, move_from, move_to):
        ...
        self.m["media"][move_to - 1], self.m["media"][move_from - 1] = (
            self.m["media"][move_from - 1],
            self.m["media"][move_to - 1],
        )
        (
            self.m["media"][move_to - 1]["order"],
            self.m["media"][move_from - 1]["order"],
        ) = (
            self.m["media"][move_from - 1]["order"],
            self.m["media"][move_to - 1]["order"],
        )
    ...
    def change_media_order(self):
        ...
        for individual_media in self.m["media"]:
            print(
                f"File Type: {individual_media['file_type']}, Current Order: {individual_media['order']}"
            )
            while True:
                try:
