...
class TestConfig(unittest.TestCase):
    ...
    def test_initialization(self):
        ...
        self.assertEqual(config["private_key"], self.private_key)
        self.assertEqual(config["eth_rpc_endpoint"], self.eth_rpc_endpoint)
        self.assertEqual(config["wallet_index"], self.wallet_index)
        self.assertEqual(config["ipfs_endpoint"], self.ipfs_endpoint)
        self.assertEqual(config["mpe_contract_address"], self.mpe_contract_address)
        self.assertEqual(config["token_contract_address"], self.token_contract_address)
        self.assertEqual(config["registry_contract_address"], self.registry_contract_address)
        self.assertEqual(config["signer_private_key"], self.signer_private_key)
        self.assertEqual(config["lighthouse_token"], " ")
        self.assertFalse(config["concurrency"])
        self.assertTrue(config["force_update"])

    def test_default_values(self):
        config = Config(private_key=self.private_key, eth_rpc_endpoint=self.eth_rpc_endpoint)

        self.assertEqual(config["wallet_index"], 0)
        self.assertEqual(config["ipfs_endpoint"], "/dns/ipfs.singularitynet.io/tcp/80/")
    ...
    def test_get_ipfs_endpoint(self):
        config = Config(private_key=self.private_key, eth_rpc_endpoint=self.eth_rpc_endpoint)
        self.assertEqual(config.get_ipfs_endpoint(), "/dns/ipfs.singularitynet.io/tcp/80/")
        ...
        self.assertEqual(config_with_custom_ipfs.get_ipfs_endpoint(), self.ipfs_endpoint)
